<template>
  <main class="visualizer-panel">
    ...
    <section class="visualizer-controls">
      ...
      <label v-else-if="algorithm === 'topological'">选择起点
        <select v-model="topologicalStart" @change="reset">
          <option v-for="node in topologicalStartOptions" :key="node.id" :value="node.id">从 {{ node.id }} 开始</option>
        </select>
      </label>
    ...
    <section class="visual-stage">
      ...
      <div v-else-if="algorithm === 'robber'" class="array-stage multi-row-stage robber-stage">
        <div class="algorithm-problem"><strong>题目描述</strong><span>一排房屋里每间都有一笔钱，相邻房屋不能同时抢。怎样选择房屋，才能拿到最多的钱？</span></div>
      ...
      <div v-else-if="algorithm === 'knapsack'" class="knapsack-stage">
        ...
        <div class="dp-table-wrap"><table class="dp-table"><thead><tr><th>物品/容量</th><th v-for="capacity in currentStep.capacity + 1" :key="capacity">{{ capacity - 1 }}</th></tr></thead><tbody><tr v-for="(row, rowIndex) in currentStep.table" :key="rowIndex"><th>{{ rowIndex === 0 ? '0 件' : `前 ${rowIndex} 件` }}</th><td v-for="(value, colIndex) in row" :key="colIndex" :class="{ filled: currentStep.filled?.[rowIndex]?.[colIndex], active: rowIndex === currentStep.row && colIndex === currentStep.col, source: rowIndex === currentStep.row - 1 && (colIndex === currentStep.col || colIndex === currentStep.fromCol) }">{{ value }}</td></tr></tbody></table></div>
      ...
      <div v-else-if="algorithm === 'unionFind'" class="union-stage">
        <svg class="union-tree" viewBox="0 0 650 270" role="img" aria-label="并查集父节点连接关系">
    ...
    <section v-else class="visualizer-empty" aria-label="算法可视化使用提示">
      <div class="visualizer-empty-mark" aria-hidden="true">🎞️</div>
      <h3>把算法的每一步看清楚</h3>
      <p>先选择一个算法，逐步观察变量、队列、指针或状态如何变化。</p>
      <div class="visualizer-empty-suggestions">
        <button type="button" @click="selectAlgorithm('recursion')">🪜 递归</button>
        <button type="button" @click="selectAlgorithm('binary')">🔎 二分查找</button>
        <button type="button" @click="selectAlgorithm('bfs')">🗺️ BFS 迷宫</button>
        <button type="button" @click="selectAlgorithm('knapsack')">🎒 背包 DP</button>
      </div>
    </section>
  </main>
</template>
...
const topologicalStartOptions = topologicalNodes.filter(node => !topologicalEdges.some(edge => edge.to === node.id));
...
const steps = computed(() => {
  ...
  return advancedSteps(algorithm.value, { topologicalStart: topologicalStart.value });
});
...
.visualizer-empty { display: flex; flex: 1; min-height: 320px; align-items: center; flex-direction: column; justify-content: center; padding: 48px 20px; color: #475569; text-align: center; }
.visualizer-empty-mark { display: grid; width: 58px; height: 58px; place-items: center; margin-bottom: 16px; border: 2px solid #a5b4fc; border-radius: 18px; background: #eef2ff; color: #4f46e5; font-size: 27px; }
.visualizer-empty h3 { margin: 0 0 8px; color: #1e293b; font-size: 24px; }
.visualizer-empty p { max-width: 440px; margin: 0; font-size: 15px; line-height: 1.7; }
.visualizer-empty-suggestions { display: flex; flex-wrap: wrap; justify-content: center; gap: 8px; margin-top: 24px; }
.visualizer-empty-suggestions button { padding: 8px 12px; border: 1px solid #c7d2fe; border-radius: 8px; background: #fff; color: #4338ca; font: inherit; font-size: 13px; cursor: pointer; }
.visualizer-empty-suggestions button:hover { border-color: #818cf8; background: #fff; }
...
.visualizer-controls input, .visualizer-controls select { width: 170px; padding: 9px 10px; border: 1px solid #cbd5e1; border-radius: 6px; background: #fff; color: #334155; font: inherit; }
...
@media (max-width: 760px) { .visualizer-panel { padding: 10px 12px; overflow-y: auto; }.visualizer-header { align-items: stretch; flex-direction: column; gap: 7px; }.visualizer-controls { align-items: stretch; flex-direction: column; }.algorithm-tabs { display: flex; width: 100%; flex: none; flex-wrap: wrap; }.algorithm-tabs button { width: auto; padding: 7px 9px; font-size: 13px; }.reset-button { margin-left: 0; }.embedded-visualizer { min-height: 560px; }.backtrack-workbench { grid-template-columns: 1fr; }.factorial-main { grid-template-columns: 1fr; }.backtrack-tree { min-height: 230px; }.number-row, .binary-arrows { grid-template-columns: repeat(4, 1fr); }.arrow-cell span { font-size: 16px; }.pointer-row { gap: 10px; font-size: 16px; }.array-line { grid-template-columns: 1fr; }.teaching-array { grid-template-columns: repeat(auto-fit, minmax(48px, 1fr)); }.row-label-spacer { display: none; }.prefix-array { grid-template-columns: repeat(6, minmax(0, 1fr)); }.house-row, .robber-stage .teaching-array { grid-template-columns: repeat(5, minmax(0, 1fr)); }.item-row { grid-template-columns: 1fr; }.item-row span { height: 96px; }.data-cell { min-height: 62px; }.data-cell strong { font-size: 18px; }.calculation-strip, .decision-strip, .sequence-strip, .graph-status, .trie-status { align-items: stretch; flex-direction: column; }.topology-status { grid-template-columns: 1fr; }.maze-grid { grid-template-columns: repeat(6, 38px); }.maze-cell { height: 38px; }.bar { width: 42px; }.bar-row { gap: 8px; } }
