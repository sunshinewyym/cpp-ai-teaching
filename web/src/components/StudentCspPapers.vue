  ...
  <div class="paper-page">
    ...
    <section v-else class="assignment-list">
      <button v-for="item in assignments" :key="item.id" class="assignment" @click="openAssignment(item.id)">
        ...
        <b v-if="item.analysisReleasedAt" class="released">解析已开放</b><b v-else>等待完成</b>
      </button>
    </section>
    ...
    <div v-if="paper" class="paper-workspace">
      ...
      <article v-if="currentQuestion" class="question-card">
        ...
        <div class="markdown" v-html="renderMd(isChoiceQuestion(currentQuestion) ? currentQuestion.question : (currentQuestion.description || currentQuestion.statement || ''))"></div>
...
function label(id) { const q = choiceMap.get(id) || programMap.get(id); const type = (isChoiceQuestion(q) ? 'choice' : (q?.type || (id.includes('-reading-') ? 'reading' : 'completion'))); return `${type === 'choice' ? '选择题' : type === 'reading' ? '阅读' : '完善'} ${q?.number || id.match(/-(\d+)$/)?.[1] || ''}`; }
